...
const RotaryPalette = ({ onColorSelect }) => {
  ...
  const handleRotate = () => setRotation((prev) => prev + 60)

  return (
    <div className="fixed top-4 left-4 z-50">
      <div className="relative">
        <Button variant="outline" size="icon" className="w-12 h-12 rounded-full shadow-lg" onClick={() => setIsOpen(!isOpen)}>
          <Palette className="h-5 w-5" />
        </Button>
        {isOpen && (
          <div className="absolute top-0 left-0 w-32 h-32">
            <div className="relative w-full h-full transition-transform duration-500 ease-in-out" style={{ transform: `rotate(${rotation}deg)` }}>
              {colors.map((color, index) => {
                ...
                return (
                  <button
                    ...
                    style={{ backgroundColor: color, left: `calc(50% + ${x}px - 16px)`, top: `calc(50% + ${y}px - 16px)` }}
                  />
                )
              })}
            </div>
            <Button variant="ghost" size="icon" className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full" onClick={handleRotate}>
      ...
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute top-0 right-0 w-32 h-32">
            ...
            <Button variant="ghost" size="icon" className="absolute top-1/2 right-1/2 transform translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full" onClick={handleRotate}>
              <RotateCcw className="h-3 w-3" />
            </Button>
          </div>
        )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 relative">
      ...
      <div className="fixed bottom-4 left-4 z-40">
        <div className="w-8 h-8 rounded-full border-2 border-white shadow-lg" style={{ backgroundColor: selectedColor }} title="Selected color" />
      </div>
    </div>
  )
}
