...
<#
.SYNOPSIS
    เปิด Unified File System MCP Chat App
.DESCRIPTION
    เปิดแอปแชตแบบรวมศูนย์ที่เชื่อมต่อทุกอย่างอัตโนมัติ
#>

Write-Host "🚀 เปิด Unified File System MCP Chat App..." -ForegroundColor Cyan

# ตรวจสอบไฟล์
if (-not (Test-Path "unified_chat_app.py")) {
    Write-Host "❌ ไม่พบไฟล์ unified_chat_app.py" -ForegroundColor Red
    Write-Host "💡 กำลังสร้างไฟล์..." -ForegroundColor Yellow
    
    # สร้างไฟล์ unified_chat_app.py
    $unifiedContent = @'
#!/usr/bin/env python3
"""
...
Please provide a helpful response in Thai language.
"""
            ...
            if response.status_code == 200:
                return response.json().get('response', 'ไม่สามารถประมวลผลได้')
            else:
                raise Exception("Ollama request failed")
                
        except Exception as e:
            return f"ไม่สามารถเชื่อมต่อ AI ได้: {str(e)}"
...
'@
    ...
    Set-Content -Path "unified_chat_app.py" -Value $unifiedContent -Encoding UTF8
    Write-Host "✅ สร้างไฟล์ unified_chat_app.py สำเร็จ" -ForegroundColor Green
}

# เช็ค Virtual Environment
if (Test-Path "venv\Scripts\Activate.ps1") {
    Write-Host "📦 ใช้ Virtual Environment" -ForegroundColor Green
